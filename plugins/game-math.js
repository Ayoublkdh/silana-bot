let handler = async (m, { conn, args, usedPrefix, command }) => {
  ...
  ⚡ المستويات المتاحة في اللعبه : 
  
${Object.keys(modes).join(' | ')} 

_📌مثال : ${usedPrefix+command} صعب_
`.trim()
...
`.trim()
  ...
  let id = m.chat
    if (id in conn.math) return conn.reply(m.chat, '❗ لا تزال هناك أسئلة دون إجابة في هذه الدردشة', conn.math[id][0])
